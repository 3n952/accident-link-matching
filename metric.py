...
def recall_topN(n: int, ground_truth: str, prediction_df: pd.DataFrame) -> float:
    ...
    정답 링크아이디가 포함되어 있는지로 Recall@N 계산
...
def evaluate(data: pd.DataFrame, processor_instance: AccidentMapMatchingProcessor, total_volume: int):
    ...
    for idx, (_, row) in enumerate(data.iterrows()):
        ...
        taas_df = row_df[['acdnt_dd_dc', 'occrrnc_time_dc', 'x_crdnt_crdnt', 'y_crdnt_crdnt']].copy()
        ...
        print(taas_df.head(5))
        with contextlib.redirect_stdout(io.StringIO()), contextlib.redirect_stderr(io.StringIO()):
            processor_instance.taas_sample_gdf = gpd.GeoDataFrame(
                taas_df,
                geometry=gpd.points_from_xy(taas_df['x_crdnt_crdnt'], taas_df['y_crdnt_crdnt']),
                crs="EPSG:5179")
            ...
            try:
                ...
                invalid_count += 1
                logging.info('해당 테스트 샘플은 데이터가 부족하여 정확한 점수산출이 안됩니다. 다음 샘플로 넘어갑니다.')
                logging.info(f'총{idx+1}개 샘플 중 {invalid_count}개 PASSED')
                continue
        ...
        logging.info(f'[{idx + 1}번째 샘플까지의 누적 평가지표 결과]\trecall@5: {metric_top5}\trecall@1: {metric_top1}')
...
if __name__ == '__main__':
    ...
    for i in range(7):
        ...
        result_top5_list.append(metric_result_top5)
        sample_num_list.append(total_volume)

    print(f'sample nums: {sample_num_list}')
    print(f'recall@1 results: {result_top1_list}')
    print(f'recall@5 results: {result_top5_list}')
